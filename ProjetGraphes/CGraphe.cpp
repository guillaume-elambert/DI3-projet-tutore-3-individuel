 ...
 * Création d'un graphe correctement initialisé à partir d'une chîane de caractère.
...
CGraphe::CGraphe(const char *cpContenu)
{
	...
	if (std::regex_match(cpContenu, cmMatchGlobal, rRegex)) {
			...
			default:
				...
				case 2:
					...
					if (iNbSommets == 0 && iNbArcs != 0) {
						throw CException(Erreur_NbArcs, "CGraphe::CGraphe(const char *cpContenu) : Le nombre de sommets a été défini sur 0, le nombre d'arcs devrait l'être aussi.\n");
					}
		...
		throw CException(Erreur_Syntaxe, "CGraphe::CGraphe(const char *cpContenu) : La chaîne de caractères ne correspond pas au format attendu\n");
	}
